...
def q1_time(file_path: str) -> List[Tuple[date, str]]:
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            # Verificar si la fecha ya está en el diccionario, si no, inicializar un contador para esa fecha
            if date not in tweets_count:
                tweets_count[date] = Counter()
    ...
    for date, users_count in sorted(tweets_count.items(), key=lambda x: sum(x[1].values()), reverse=True)[:10]:
        ...
        top_user = users_count.most_common(1)[0][0]
